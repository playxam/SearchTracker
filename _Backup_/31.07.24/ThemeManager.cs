...
public static class ThemeManager
{
    ...
    public static void ApplyTheme(string themeName)
    {
        ...
        // Überprüfen, ob der Theme-Name nicht leer ist und ob das Theme verfügbar ist
        if (string.IsNullOrEmpty(themeName) || !GetAvailableThemes().Contains(themeName))
        {
            Logger.Log($"Error: Theme '{themeName}' is not available or is empty.");
            MessageBox.Show($"Das Theme '{themeName}' ist nicht verfügbar oder leer. Bitte wählen Sie ein gültiges Theme.");
            throw new ArgumentException($"Theme '{themeName}' is not available or is empty.");
        }
        ...
        {
            // Anwenden des entsprechenden Themes
            switch (themeName)
            {
                case "Standard":
